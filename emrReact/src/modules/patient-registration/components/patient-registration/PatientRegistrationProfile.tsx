...
import { Typography, Card, CardContent, Avatar, IconButton, Badge, Divider, CardHeader, Box, Slider } from '@mui/material';
...
import { PatientDashboardProfileForm } from 'modules/medical-examination/components/patient-dashboard/PatientDashboardProfileForm';
...
export const PatientRegistrationProfile = (props: PatientBlockProps) => {
  ...
  return (
    <>
      <Card className={styles.profileCard}>
        {data && <CardHeader action={<PatientInfoForm isCreate={false} data={data} />} />}
        {!data && (
          <CardHeader title={t('patientRegistration.patientInfoTitle')} action={<PatientDashboardProfileForm data={data} />} />
        )}
        ...
        {data && (
          <>
            <CardContent sx={{ padding: '12px 12px 0 12px' }}>
              <Avatar
                sx={{ width: 80, height: 80, top: '-14px !important' }}
              ...
              <FlexBox justifyContent='center' sx={{ marginBottom: '0 !important' }} className={styles.profileSubheader}>
                <Badge color='error' badgeContent={0}>
                  <SmartphoneIcon sx={{ color: 'text.disabled', fontSize: '1.3rem' }} />
                </Badge>
                <Typography component='span' sx={{ marginBottom: '0 !important' }} className={styles.profileSubheader}>
                  {data?.Phone}{' '}
                </Typography>
              </FlexBox>
              <FlexBox justifyContent='center' sx={{ marginBottom: '0 !important' }} className={styles.profileSubheader}>
      ...
      <Card sx={{ marginBottom: '16px',minHeight:"52vh" }}>
        <CardHeader
          title={t('patientRegistration.physicalConditionTitle')}
          action={<PatientDashboardProfileForm data={data} />}
        />
        <CardContent sx={{ padding: '12px 12px 0 12px' }}>
          {data && (
